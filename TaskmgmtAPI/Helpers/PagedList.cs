...
namespace TaskmgmtAPI.Helpers
{
    public class PagedList<T> : List<T>
    {
        ...
        public static PagedList<T> ToPagedList(IQueryable<T> source, int pageNumber, int pageSize,HttpContext httpctxt)
        {
            var count = source.Count();
            var items = source.Skip((pageNumber - 1) * pageSize).Take(pageSize).ToList();
            return new PagedList<T>(items, count, pageNumber, pageSize, httpctxt);
        }
    }
}
